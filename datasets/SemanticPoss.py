...
# ----------------------------------------------------------------------------------------------------------------------
#
#      Class handling SemanticPoss dataset.
...
class SemanticPossDataset(PointCloudDataset):
    """Class to handle SemanticPoss dataset."""

    def __init__(self, config, set='training', balance_classes=True, seqential_batch = False):
        PointCloudDataset.__init__(self, 'SemanticPoss')
        ...
        if self.set == 'training':
            self.sequences = ['{:02d}'.format(i) for i in [1, 4, 5]]
        elif self.set == 'validation':
            self.sequences = ['{:02d}'.format(i) for i in [0]]
        elif self.set == 'test':
            self.sequences = ['{:02d}'.format(i) for i in [2]]
        else:
            raise ValueError('Unknown set for SemanticPoss data: ', self.set)
        ...
        if config.n_frames == 1:
            config_file = join(self.path, 'semantic-poss.yaml')
            ...
            while num_merged < self.config.n_frames and f_ind - f_inc >= 0:
                ...
                if self.set in ['training', 'validation']  and f_inc > 0 and  self.config.n_test_frames == 1:#during training
                    ...
                    if self.config.sampling == 'objectness':
                        mask = ((sem_labels > 0) & (sem_labels < 4) & mask)
                    elif self.config.sampling == 'importance':
                        n_points_to_sample = np.sum((sem_labels > 0) & (sem_labels < 4))
                ...
                if self.set in ['validation', 'test'] and self.config.n_test_frames > 1 and f_inc > 0:
                    ...
                    if self.config.sampling == 'objectness':
                        ...
                        if label_pred is not None:
                            mask = ((label_pred > 0) & (label_pred < 4) & mask)
                    elif self.config.sampling == 'importance':
...
class SemanticPossSamplerTest(Sampler):
    """Sampler for SemanticPoss for testing"""

    def __init__(self, dataset: SemanticPossDataset):
...
class SemanticPossSampler(Sampler):
    """Sampler for SemanticPoss"""
...
class SemanticPossCustomBatch:
    """Custom batch definition with memory pinning for SemanticPoss"""
...
def SemanticPossCollate(batch_data):
    return SemanticPossCustomBatch(batch_data)
